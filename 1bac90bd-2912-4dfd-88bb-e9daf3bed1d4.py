...
def __setMetaData__(track: Track, album: Album, filepath, contributors, lyrics):
    ...
    # Maintain exact artist format using ", "
    obj.artist = ", ".join([artist.name for artist in track.artists])
...
async def downloadTrack(track: Track, album=None, playlist=None, userProgress=None, partSize=1048576, \
    ...
    try:
        ...
        # Rename file if it contains '_converted'
        if '_converted' in converted_path:
            final_path = converted_path.replace('_converted', '')
        ...
        try:
            ...
            if SETTINGS.lyricFile:
                lrcPath = final_path.rsplit(".", 1)[0] + '.lrc'
